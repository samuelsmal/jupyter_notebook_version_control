...
def get_notebook_data(path_to_file):
    with codecs.open(path_to_file, 'r', 'utf-8') as notebook:
...
def write_notebook_data_to_py(notebook_data, out_file_path):
    with codecs.open(out_file_path, 'w', 'utf-8') as output:
...
def create_git_commit(os_path):
    ...
    check_call(split('git add {0}'.format(filename)), cwd=workdir)
    check_call(split('git commit -m "[NOTEBOOK SNAPSHOT]:" {0}'.format(filename)), cwd=workdir)
